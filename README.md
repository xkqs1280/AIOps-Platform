...
为什么需要它：设备本地日志缓冲区通常只有几百条且**重启即丢**，无法满足等保 2.0 三级 /《网络安全法》第 21 条"网络日志留存 ≥ 6 个月"。平台侧按 180 天留存（`DEVICE_LOGS_DAYS`），是这一要求唯一可落地的技术手段。
...
| `DEVICE_LOGS_DAYS` | 设备日志留存天数，默认 `180`（等保 ≥ 6 个月，不要调小） |
