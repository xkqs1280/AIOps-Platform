...
设备一多，靠人盯班就是一场灾难：告警刷屏看不过来、配置改动没记录、巡检报告熬夜手写、故障定位全凭经验。AIOps 平台专门解决这些问题——一套系统管住「看得见、报得准、查得快、备得上」。以下全部为平台真实运行界面截图（v4.5.7）。
...
- 🆕 **设备日志中心新增接收总开关**：可在页面上自助开启/关闭平台接收设备日志 —— 关闭即**释放 UDP 514 端口**（把端口让给现场其它程序），存量日志照常检索与导出、不受影响；开关状态持久化（重启保持），仅管理员可操作并写审计
- 🆕 **访问 IP 白名单**（账号管理页）：可自定义「哪些 IP 能访问本平台」，支持单个 IP 与网段（如 `192.168.1.0/24`）并附备注，未授权来源**整站拦截**（连前端页面一起拦，只显示一页带其自身来源 IP 的提示）；三道防自锁设计 —— 保存时强校验名单必须包含操作者当前 IP（否则拒绝落库）、**回环地址永久放行**（名单配错也能从服务器本机改回来）、开启前二次确认。默认关闭，老部署行为完全不变；设备日志/Trap 上报端点与 `/health` 豁免（否则设备日志会静默中断）
- 🔧 **监控大屏「带宽利用率 TOP10」改为「接口流量 TOP10」**：排名口径由「每台设备取利用率最高接口、按百分比排序」改为「**全平台所有接口按收发总流量降序**」，避免"1G 口跑 80% 上榜、10G 口跑 3G 落榜"的口径失真；同台设备多个接口可同时上榜，x 轴改为速率自适应（Mbps/Gbps），tooltip 保留收发明细与利用率
- 🆕 **监控大屏标题可自定义**：系统设置 → 显示设置 中可将「AIOps 网络监控平台」改为任意名称（如「某某单位网络监控平台」），大屏每 60 秒自动跟随、无需手动刷新；留空则回退默认标题
