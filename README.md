...
设备一多，靠人盯班就是一场灾难：告警刷屏看不过来、配置改动没记录、巡检报告熬夜手写、故障定位全凭经验。AIOps 平台专门解决这些问题——一套系统管住「看得见、报得准、查得快、备得上」。以下全部为平台真实运行界面截图（v4.5.6）。
...
平台**自己监听 UDP 514** 收 syslog —— 不需要另装 rsyslog/snmptrapd，开箱即用。设备日志按 H3C Comware / 华为 VRP 格式解析成 模块 / 助记符 / 级别 / 分类 / 接口 / 操作用户，支持多条件检索、趋势与分布统计、CSV 导出；安全类日志同时进「安全监控」面板。

配套**日志主机一键下发**：先预览命令 → 确认后平台读取设备 info-center 配置快照 → 逐条下发并回验 → 全程留痕、可一键回滚（不勾选"保存到启动配置"则只做运行时下发，便于灰度）。

为什么需要它：设备本地日志缓冲区通常只有几百条且**重启即丢**，无法满足等保 2.0 三级 /《网络安全法》第 21 条"留存相关的网络日志不少于六个月"。平台侧默认留存 180 天（`DEVICE_LOGS_DAYS`，可在 `.env` 调大——注意"6 个月"按自然月是 181~184 天），是这一要求唯一可落地的技术手段。

![设备日志中心：设备 syslog 统一接收 + 检索 + 统计 + 导出](docs/screenshots/12-device-logs.png)

![日志主机下发：命令预览 + 批量下发/回滚 + 配置状态留痕](docs/screenshots/13-device-logs-loghost.png)

![日志详情抽屉：列表滚到最底部时依然贴视口显示（原始报文可复制，作为审计依据）](docs/screenshots/14-device-logs-detail.png)

## v4.5.6 更新要点

- 🐛 **修复「日志主机回验假阴性」**（现场：设备上明明配好了、日志也一直在发，平台却报"回验未在配置中找到目标"= 失败）
  - **回验改读运行时真值 `display info-center`**（首选命令）：它直接回答"日志现在到底发去哪了"，不依赖 `| include` 过滤语法、也不受配置是否落盘影响；原来的配置类命令降为兜底。判定失败时会把"设备当前生效的日志主机"一并写进消息，不必再爬上设备复核
  - **「读不到配置」不再等同于「下发失败」**：回验结果由二态改为三态 —— `已下发 / 待确认 / 失败`。回读不可信（命令报错到底、空回读）判为**待确认**（页面黄色徽标），只有"回读可信且确实没有目标行"或"下发命令报错"才判失败；回滚侧对称处理（空回读不再冒充"已回滚成功"）
  - 回读链四个健壮性修复：行分隔兼容 `CRLF / CR / LF`（老设备只发裸 CR 时原实现会把整段配置粘成一行，行首匹配全灭）、剥离 ANSI 控制序列、分页标记扩到三种并支持跨 TCP 分片识别、`| include` 报错时自动退化到更宽的读取方式
- 🆕 **顶部新增「日志主机」卡片**：一键查看已接入日志的设备（设备名 + IP + 来源 + 最近收到日志的时间与条数）。口径为「**日志确实在进来的** ∪ **平台确认下发但暂无日志的**」——收到日志是"配置真的生效"唯一硬证据，避免把"记录里写着失败、日志却一直在进"的设备漏掉
- 🆕 **下发页选择设备时自动标注「已下发」**：除下发记录外，并入近 7 天真实收到的日志，仅在设备上手工配好而平台无记录的设备也会显示「已下发」，**避免误以为没配上而重复下发**（对生产设备等于二次真实变更）
- 🐛 **修复「批量下发/回滚 70 台设备报 500，前端只看到一句 Request failed with status code 500」**：根因是单台设备的会话被中断时，`asyncio.CancelledError` 自 Python 3.8 起继承 `BaseException`，同时绕过了内层 `except Exception` 与批量层的异常判定，最终在解包处抛 `TypeError` 导致**整批失败** —— 而设备配置其实已经改了一半，操作人很可能重复下发。现改为「每台设备恰好产出一条结果」，单台异常按本台失败如实返回；同时降低单请求批量（30 → 10 台/批）以降低长请求被中途断开的概率
- 🛡 **syslog 接收器绑定失败改为平台自诊断**：原先无论什么原因都提示 "Linux 下绑定 < 1024 端口需 CAP_NET_BIND_SERVICE"，**在 Windows 生产上是误导**（真实原因是端口被别的程序占用）。现按操作系统分支：Windows 直接报出占用该 UDP 端口的**进程名与 PID**，并区分端口已占用（`WSAEADDRINUSE`）/ 系统保留区间 / 地址非本机三种情形
- 🐛 **修复服务宿主日志目录定位**：以 `__file__` 推导日志目录在 onefile 冻结后会指向临时解包目录（进程退出即被清理），日志写完即消失、现场按文档找不到；改为以可执行文件所在目录为基准

## v4.5.5 更新要点

- 🆕 **设备日志中心**：内置 syslog UDP 接收器（受监督、崩溃自愈、批量入库、按来源 IP 限流），H3C / 华为真机报文解析（级别取自消息体 `/N/` 而非不可靠的 PRI），多条件检索 + 小时趋势 + 分类分布 + CSV 导出，日志留存 180 天
- 🆕 **日志主机批量下发与回滚**：预览 → 确认 → 快照 → 逐条下发并检查设备错误标记 → 回验 → 落库留痕，可一键 `undo`；支持不保存灰度
- 🛡 **审计数据不可销毁**：删除设备时日志与变更记录**只解除关联、绝不删除**（并保留设备名/IP 快照），杜绝"删设备即销毁证据"
- 🐛 **修复真机报文 NUL 字节导致整批日志丢失**：H3C 部分报文尾部带 `0x00`，PostgreSQL 的 text 类型不接受，会让整批（最多 500 条）INSERT 一起失败；现已在接收入口与解析层双重剥离，并在批量插入失败时逐条重试，避免一条坏报文毁掉整批
- 🔧 **修复 syslog 接收器设备缓存时间戳未复位**导致日志关联不上设备的问题
- 🐛 **修复弹层被"页面进入动效"劫持定位**（影响面涉及全部使用该动效的页面）：页面根节点的进场动画用了 `animation-fill-mode: both`，而其关键帧含 `transform`，Chrome 会因此让该节点**长期持有** `transform: matrix(1,0,0,1,0,0)`（即使终态写的是 `none` 也不还原），使它成为 `position: fixed` 后代的包含块 —— 抽屉/弹窗改为相对**页面内容盒**定位。表现为：列表滚到底部后点「日志详情」，要滚回顶部才看得到内容。已改为 `backwards` 填充，动画结束即无残留，动效视觉不变
- 📦 **Linux 包 systemd 单元新增 `AmbientCapabilities=CAP_NET_BIND_SERVICE`**（绑定 514 特权端口所需，仅授该单项能力，不以 root 运行后端）
- 🆕 **华为 VRP 适配**：日志主机的下发命令按厂商分支生成 —— 华为不支持 `save force`（会被当作文件名），改用 `save`；源接口语法为 `info-center source default channel loghost log level <级别>`（H3C 为 `info-center source default loghost level <级别>`）；解析侧兼容华为 `Sep 15 2026 21:12:23` 的时间戳格式（年在前），并修正 OID 段被误判为源 IP 的问题
- 🛡 **接入端点补齐 IP 限流**：`/device-logs/ingest` 与 `/syslog`、`/traps` 对齐（120 次/分/IP），防突发打爆接收队列
- 🐛 **修复设备 CLI 终端退格键失效**：xterm 固定发送 `DEL(0x7F)`，而 telnet 通道完成选项协商后华为设备不再把 `0x7F` 当退格（回显 BEL、字符不删）；后端在协议适配层统一归一为 `BS(0x08)`
- 🐛 **修复长耗时设备操作被全局 15s 超时误杀**：批量备份 / 合规核查 / 终端探活等串行循环接口按路径放宽超时（普通查询仍保持 15s）
- 🔧 **排查并修复真机时钟偏移**：定位到部分华为设备时区被配成 `UTC-08:00`（而设备 syslog 报文取的是 RTC 内部值），导致平台「设备时间」比「接收时间」快 8 小时；已给出对齐 H3C 的修复命令序列
...
- **设备日志中心**：内置 syslog UDP 接收（无需外部 rsyslog）、H3C/华为报文解析、检索/统计/导出、日志主机一键下发与回滚、180 天留存
- **H3C 巡检**：一键巡检任务与评分
...
| `SYSLOG_UDP_PORT` | 设备日志中心接收端口（默认 `514`，Linux 绑 <1024 需 systemd `AmbientCapabilities=CAP_NET_BIND_SERVICE`） |
| `SYSLOG_ADVERTISE_ADDRESS` | 平台侧地址，设备通过它回传日志；多网卡主机建议显式指定 |
| `SYSLOG_DEVICE_TZ_OFFSET_HOURS` | 设备时钟相对 UTC 的偏移（设备按北京时间设置时填 `8`；原始时间串始终保留） |
| `DEVICE_LOGS_DAYS` | 设备日志留存天数，默认 `180`。等保要求"不少于六个月"（自然月 181~184 天），需严格达标请调大到 `200`/`365` |
