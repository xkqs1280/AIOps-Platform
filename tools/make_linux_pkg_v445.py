"""制作 AIOps v4.4.5 Linux 生产包（源码部署）"""
import io
import os
import shutil
import zipfile
...
STAGE = os.path.join(ROOT, "dist", "aiops-v4.4.5-linux")
PKG_DIR = os.path.join(STAGE, "AIOps")
OUT = os.path.join(ROOT, "dist", "aiops-v4.4.5-linux.zip")
...
# .env.example
env_example = """# AIOps v4.4.5 环境配置模板（复制为 .env 后按需修改）
# PostgreSQL 连接串
...
if pgrep -f '[u]vicorn app.main:app' >/dev/null; then
  ...
  echo "AIOps v4.4.5 已启动: http://${IP:-127.0.0.1}:8000  （日志: backend/uvicorn.log）"
else
...
## 二、Linux 部署（源码包 `aiops-v4.4.5-linux.zip`）
...
```bash
# 1. 解压
unzip aiops-v4.4.5-linux.zip && cd AIOps
