"""制作 AIOps v4.5.5 Linux 生产包（源码部署）"""
import io
import os
import shutil
import zipfile
...
STAGE = os.path.join(ROOT, "dist", "aiops-v4.5.5-linux")
PKG_DIR = os.path.join(STAGE, "AIOps")
OUT = os.path.join(ROOT, "dist", "aiops-v4.5.5-linux.zip")
...
# .env.example
env_example = """# AIOps v4.5.5 环境配置模板（复制为 .env 后按需修改）
# PostgreSQL 连接串
...
# 离线配置合规巡检 CLI（零依赖单文件）：规则集由脚本自动定位到
# <脚本目录>/../backend/app/data/config_baseline_rules.json，故置于 tools/ 即可开箱可用。
print("== 复制离线巡检 CLI ==")
os.makedirs(os.path.join(PKG_DIR, "tools"), exist_ok=True)
_cli = os.path.join(ROOT, "tests", "config_audit.py")
if os.path.isfile(_cli):
    shutil.copy2(_cli, os.path.join(PKG_DIR, "tools", "config_audit.py"))
    print("  tools/config_audit.py 已复制")
else:
    print("  [!] 未找到 tests/config_audit.py")
...
[Unit]
Description=AIOps Platform Backend (v4.5.5)
...
# 设备日志中心的内置 syslog 接收器要绑定 UDP 514（特权端口）。
# AmbientCapabilities 只授予绑定低端口的单项能力，不必以 root 运行整个后端；
# 若不需要 514 也可在 .env 把 SYSLOG_UDP_PORT 改到 >=1024 来避免使用特权端口。
...
if command -v systemctl >/dev/null 2>&1 && [ -f "/etc/systemd/system/${SVC}.service" ]; then
  ...
  if $SUDO systemctl restart "$SVC" >/dev/null 2>&1; then
    ...
    if $SUDO systemctl is-active --quiet "$SVC"; then
      IP=$(hostname -I 2>/dev/null | awk '{print $1}')
      echo "AIOps v4.5.5 已启动: http://${IP:-127.0.0.1}:8000"
...
if [ "$OK" = 1 ]; then
  IP=$(hostname -I 2>/dev/null | awk '{print $1}')
  echo "AIOps v4.5.5 已启动: http://${IP:-127.0.0.1}:8000  （日志: backend/uvicorn.log）"
else
...
## 二、Linux 部署（源码包 `aiops-v4.5.5-linux.zip`）
...
```bash
# 1. 解压
unzip aiops-v4.5.5-linux.zip && cd AIOps
