"""制作 AIOps v4.5.7 Linux 生产包（源码部署）"""
import io
import os
import shutil
import zipfile
...
STAGE = os.path.join(ROOT, "dist", "aiops-v4.5.7-linux")
PKG_DIR = os.path.join(STAGE, "AIOps")
OUT = os.path.join(ROOT, "dist", "aiops-v4.5.7-linux.zip")
...
# .env.example
env_example = """# AIOps v4.5.7 环境配置模板（复制为 .env 后按需修改）
# PostgreSQL 连接串
...
[Unit]
Description=AIOps Platform Backend (v4.5.7)
...
if command -v systemctl >/dev/null 2>&1 && [ -f "/etc/systemd/system/${SVC}.service" ]; then
  ...
  if $SUDO systemctl restart "$SVC" >/dev/null 2>&1; then
    ...
    if $SUDO systemctl is-active --quiet "$SVC"; then
      IP=$(hostname -I 2>/dev/null | awk '{print $1}')
      echo "AIOps v4.5.7 已启动: http://${IP:-127.0.0.1}:8000"
...
if [ "$OK" = 1 ]; then
  IP=$(hostname -I 2>/dev/null | awk '{print $1}')
  echo "AIOps v4.5.7 已启动: http://${IP:-127.0.0.1}:8000  （日志: backend/uvicorn.log）"
else
...
## 二、Linux 部署（源码包 `aiops-v4.5.7-linux.zip`）
...
```bash
# 1. 解压
unzip aiops-v4.5.7-linux.zip && cd AIOps
