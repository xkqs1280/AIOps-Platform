<template>
  <div class="min-h-full p-6">
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-100">审计日志</h1>
      <p class="mt-1 text-sm text-gray-400">记录登录、设备变更、用户管理等敏感操作的完整轨迹</p>
    </div>
    ...
    <div class="mb-4 flex flex-wrap items-center gap-3">
      ...
      <input
        ...
        class="w-64 rounded-lg border border-gray-700 bg-gray-800 px-3 py-1.5 text-sm text-gray-200 placeholder-gray-500 focus:border-cyan-500 focus:outline-none"
      ...
      <button
        @click="fetchLogs"
        class="rounded-lg bg-gray-800 px-3 py-1.5 text-sm text-gray-300 border border-gray-700 hover:bg-gray-700"
      >刷新</button>
    </div>
    ...
    <div class="rounded-xl border border-gray-800 bg-gray-900/50">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="log in logs" :key="log.id" class="border-b border-gray-800/60 hover:bg-gray-800/30">
            <td class="px-4 py-2.5 text-xs text-gray-400 whitespace-nowrap">{{ formatTime(log.created_at) }}</td>
            <td class="px-4 py-2.5 font-medium text-gray-200">{{ log.user }}</td>
            ...
            <td class="px-4 py-2.5 text-gray-300">{{ moduleLabel(log.module) }}</td>
            <td class="px-4 py-2.5">
              <span
                class="inline-flex items-center rounded border px-1.5 py-0.5 text-xs font-medium"
            ...
            <td class="px-4 py-2.5 text-xs text-gray-400 max-w-md truncate" :title="log.detail">{{ log.detail || '-' }}</td>
            <td class="px-4 py-2.5 text-xs text-gray-500 font-mono">{{ log.ip || '-' }}</td>
          </tr>
          <tr v-if="logs.length === 0">
            <td colspan="7" class="px-4 py-10 text-center text-sm text-gray-500">暂无审计日志</td>
          </tr>
        </tbody>
      </table>
        ...
        <div class="flex items-center gap-3 text-sm">
          <span class="flex items-center gap-1 text-xs text-gray-500">
            ...
            <select
              ...
              class="rounded-lg border border-gray-700 bg-gray-800 px-2 py-1 text-sm text-gray-200 focus:outline-none"
            >
          ...
          <button
            ...
            :disabled="page <= 1"
            class="rounded-lg border border-gray-700 bg-gray-800 px-3 py-1 text-gray-300 hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed"
          >上一页</button>
