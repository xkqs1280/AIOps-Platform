<template>
  <div class="min-h-screen bg-gray-950 text-gray-100 p-4 flex flex-col gap-4">
    ...
    <!-- 评估方式说明 -->
    <div class="px-2 -mt-1 text-xs text-gray-500">
      ...
      <span class="inline-flex items-center gap-1">
        <span class="w-2 h-2 rounded-full bg-gray-500"></span> 平台指标推断（设备未配置 SSH 时回退）
      </span>
    </div>

    <!-- Overall Score + Radar Chart -->
    <div class="grid grid-cols-3 gap-4 h-48">
      <div class="bg-gray-900 rounded-xl border border-gray-800 p-4 flex flex-col items-center justify-center">
        <span class="text-xs text-gray-400 mb-1">整体合规评分</span>
        <span class="text-5xl font-bold" :class="scoreColor">{{ overallScore }}%</span>
        <span class="text-xs text-gray-500 mt-1">{{ overallStats.compliant_devices || 0 }} / {{ overallStats.total_devices || 0 }} 设备合规</span>
      </div>
      <div class="col-span-2 bg-gray-900 rounded-xl border border-gray-800 p-4 flex flex-col">
        <div class="flex items-center gap-2 border-l-4 border-green-400 pl-2 mb-2">
          <span class="text-sm font-semibold text-gray-200">五大类合规得分</span>
        </div>
    ...
    <div class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden flex-1">
      <div class="flex items-center justify-between px-4 py-3 border-b border-gray-800">
        ...
        <div class="flex items-center gap-3 text-xs text-gray-400">
          <label class="flex items-center gap-1.5 cursor-pointer hover:text-gray-200">
            <input type="checkbox" :checked="allSelected" @change="toggleAll" class="accent-cyan-500" />
            全选
          </label>
          <button @click="clearSelection" class="hover:text-gray-200">清空</button>
        </div>
      </div>
      <div class="overflow-x-auto">
            ...
            <template v-for="device in devices" :key="deviceKey(device)">
              ...
              >
                ...
                <td class="px-4 py-2.5">
                  <span class="text-gray-500 text-xs transition-transform inline-block mr-1" :class="expandedId === deviceKey(device) ? 'rotate-90' : ''">&#9654;</span>
                  <div class="inline-flex flex-col align-middle">
                    <span class="text-gray-200">{{ device.device_name }}</span>
                    <span class="text-xs text-gray-500 font-mono">{{ device.ip || '-' }}</span>
                  </div>
                </td>
                <td class="px-4 py-2.5">
                  <span
                    class="px-2 py-0.5 rounded text-xs font-medium"
                    :class="device.method === 'ssh_config' ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/30' : 'bg-gray-600/20 text-gray-400 border border-gray-600/30'"
                  >
                ...
                <td class="px-4 py-2.5 text-gray-400">{{ device.passed != null ? device.passed : (device.passed_checks || 0) }} / {{ device.total != null ? device.total : (device.total_checks || 0) }}</td>
                <td class="px-4 py-2.5 text-gray-500 text-xs">{{ formatTime(device.checked_at || device.last_checked) }}</td>
              </tr>
              <!-- Expanded: Control Checks Detail -->
              <tr v-if="expandedId === deviceKey(device)" class="bg-gray-800/30">
                <td colspan="6" class="px-6 py-3">
                  <div v-if="expandedLoading" class="text-gray-500 text-sm py-4 text-center">加载中...</div>
                  <div v-else-if="expandedChecks.length === 0" class="text-gray-500 text-sm py-4 text-center">暂无检查项数据</div>
                  <div v-else>
                    <!-- 分类得分 -->
                    <div v-if="expandedCategories && Object.keys(expandedCategories).length" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 mb-4">
                      <div
                        ...
                        :key="key"
                        class="bg-gray-900/60 rounded-lg px-3 py-2 border border-gray-700/50"
                      >
                    ...
                    <div class="grid grid-cols-1 gap-2 max-h-60 overflow-y-auto custom-scrollbar">
                      <div
                        ...
                        class="flex items-start justify-between gap-3 bg-gray-900/60 rounded-lg px-4 py-2 border border-gray-700/50"
                      >
                        <div class="flex items-start gap-3 min-w-0">
                          <span class="w-2 h-2 rounded-full mt-1.5 shrink-0" :class="checkStatusDot(check.status)"></span>
                          <div class="min-w-0">
                            <div class="text-sm text-gray-300">{{ check.desc || check.name || check.control_name }}</div>
                            <div v-if="check.evidence" class="text-xs text-gray-500 mt-0.5 break-words">{{ check.evidence }}</div>
                          </div>
                        </div>
        ...
        <div v-if="devices.length === 0" class="px-4 py-10 text-center text-gray-500 text-sm">暂无设备合规数据</div>
        <!-- 分页控件 -->
        <div v-if="complianceTotal > compliancePageSize" class="flex items-center justify-between px-4 py-3 border-t border-gray-800">
          <span class="text-xs text-gray-500">共 {{ complianceTotal }} 台设备</span>
          <div class="flex items-center gap-3 text-sm">
            <span class="flex items-center gap-1 text-xs text-gray-500">
              ...
              <select
                ...
                class="bg-gray-800 border border-gray-700 rounded-lg px-2 py-1 text-sm text-gray-200 focus:outline-none"
              >
            ...
            <button
              ...
              class="px-3 py-1 rounded-lg bg-gray-800 text-gray-300 border border-gray-700 hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >上一页</button>
            <span class="text-xs text-gray-400">第 {{ compliancePage }} / {{ complianceTotalPages }} 页</span>
            <button
              @click="compliancePage++; fetchStatus()"
    ...
    >
      <div class="bg-gray-900 border border-gray-700 rounded-xl w-[820px] max-h-[85vh] flex flex-col shadow-2xl">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <div>
            <h3 class="text-lg font-bold">等保合规配置示例（H3C 设备）</h3>
            <p class="text-xs text-gray-500 mt-0.5">对应平台合规检查项 SEC-1.1 ~ SEC-5.1，可直接复制到设备执行（需在系统视图下）</p>
          </div>
          <button class="px-3 py-1.5 text-xs text-gray-400 hover:text-gray-200" @click="showExamples = false">关闭</button>
        </div>
        <div class="flex-1 overflow-auto custom-scrollbar p-6 space-y-6">
          <div v-for="group in complianceExamples" :key="group.category">
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-500">{{ group.items.length }} 项</span>
              <div class="flex-1 h-px bg-gray-800"></div>
            </div>
            <div class="space-y-3">
              <div v-for="item in group.items" :key="item.id" class="bg-gray-800/40 border border-gray-700/60 rounded-lg p-3">
                <div class="flex items-start justify-between gap-3 mb-1.5">
                  <div class="flex items-center gap-2">
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-blue-900/50 text-blue-300 font-mono">{{ item.id }}</span>
                    <span class="text-sm text-gray-200">{{ item.desc }}</span>
                  </div>
                  <button
                    class="px-2 py-0.5 text-[10px] bg-gray-700 hover:bg-gray-600 rounded text-gray-300 shrink-0 transition-colors"
                    @click="copyExample(item.cmd)"
                  >
                    复制
                  </button>
                </div>
                <pre class="text-xs text-green-300/90 font-mono whitespace-pre-wrap leading-relaxed bg-gray-950/70 rounded p-2.5 overflow-x-auto">{{ item.cmd }}</pre>
              </div>
            </div>
          </div>
        </div>
...
function checkStatusDot(status) {
  const map = { compliant: 'bg-green-500', partial: 'bg-yellow-500', non_compliant: 'bg-red-500', not_applicable: 'bg-gray-500' }
...
function checkStatusBadge(status) {
  const map = {
    ...
    not_applicable: 'bg-gray-600/20 text-gray-400 border-gray-600/30',
  }
  return (map[status] || 'bg-gray-600/20 text-gray-400') + ' border'
}
