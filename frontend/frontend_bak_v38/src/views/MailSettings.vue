<template>
  <div class="p-6 max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-100">邮件告警设置</h2>
        <p class="text-sm text-gray-500 mt-1">SMTP 邮件通知：设备离线、告警规则触发及恢复时自动发送</p>
      </div>
      <span v-if="isAdmin" class="px-2 py-0.5 rounded text-xs bg-cyan-500/10 text-cyan-400 border border-cyan-500/30">管理员</span>
    </div>

    <!-- 非管理员提示 -->
    <div v-if="!isAdmin" class="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center">
      <div class="text-3xl mb-3">🔒</div>
      <p class="text-sm text-gray-400">仅管理员可查看和配置邮件告警</p>
    </div>
    ...
    <div v-else class="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-5">
      ...
      <div class="flex items-center justify-between">
        <div>
          <div class="text-sm font-medium text-gray-200">启用邮件告警</div>
          <div class="text-xs text-gray-500 mt-1">开启后自动发送设备离线 / 恢复 / 规则触发通知</div>
        </div>
        <button
          @click="mailCfg.enabled = !mailCfg.enabled"
          class="relative w-11 h-6 rounded-full transition-colors focus:outline-none shrink-0"
          :class="mailCfg.enabled ? 'bg-cyan-600' : 'bg-gray-700'"
        >
          <span
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="text-xs text-gray-500 font-medium block mb-1.5">SMTP 服务器</label>
          <input
            v-model="mailCfg.smtp_host"
            placeholder="smtp.example.com"
            class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none"
          />
        </div>
        <div>
          <label class="text-xs text-gray-500 font-medium block mb-1.5">端口</label>
          <input
        ...
        <div>
          <label class="text-xs text-gray-500 font-medium block mb-1.5">发信账号</label>
          <input
        ...
        <div>
          <label class="text-xs text-gray-500 font-medium block mb-1.5">发信密码</label>
          <input
        ...
        <div>
          <label class="text-xs text-gray-500 font-medium block mb-1.5">发件人地址</label>
          <input
        ...
        <div>
          <label class="text-xs text-gray-500 font-medium block mb-1.5">收件人地址</label>
          <input
      ...
      <p class="text-xs text-gray-600">同一事件 5 分钟内不重复发送。</p>
    </div>
  </div>
</template>
