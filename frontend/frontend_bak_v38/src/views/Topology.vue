  ...
  <div class="flex h-screen flex-col bg-gray-950 text-gray-100">
    ...
    <div class="border-b border-gray-800 bg-gray-900/50 px-6 py-4">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-100">网络拓扑可视化</h1>
          <p class="mt-1 text-sm text-gray-400">实时网络设备拓扑与链路状态</p>
        </div>
        <button
          @click="refreshTopology"
          :disabled="loading"
          class="rounded-lg border border-gray-700 bg-gray-800 px-4 py-2 text-sm font-medium text-gray-300 transition-colors hover:bg-gray-700 disabled:opacity-50"
        >
          {{ loading ? '加载中...' : '刷新' }}
        </button>
      </div>
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="relative flex-1">
        ...
        >
          <div class="text-sm text-gray-400">正在加载拓扑数据...</div>
        </div>
        ...
        >
          <div class="text-center">
            <div class="text-gray-500">暂无拓扑数据</div>
          </div>
        </div>
      </div>
      ...
      <div class="w-72 border-l border-gray-800 bg-gray-900/50 p-4">
        <h3 class="mb-3 text-sm font-semibold text-gray-300">图例</h3>

        <!-- Node Type Legend（设备类型图例已移除，类型已直接应用于节点图形） -->

        <!-- Status Legend -->
        <div class="mb-5">
          <p class="mb-2 text-xs font-medium text-gray-500">设备状态</p>
          <div class="space-y-2">
            <div class="flex items-center gap-3">
              <div class="h-3 w-3 rounded-full" style="background-color: #10b981"></div>
              <span class="text-sm text-gray-300">在线 (Online)</span>
            </div>
            <div class="flex items-center gap-3">
              <div class="h-3 w-3 rounded-full" style="background-color: #f59e0b"></div>
              <span class="text-sm text-gray-300">告警 (Warning)</span>
            </div>
            <div class="flex items-center gap-3">
        ...
        <!-- Edge Legend -->
        <div class="mb-5">
          <p class="mb-2 text-xs font-medium text-gray-500">链路利用率</p>
          <div class="space-y-2">
            <div class="flex items-center gap-3">
              <div class="h-0.5 w-8" style="background-color: #6b7280"></div>
              <span class="text-sm text-gray-300">低 (&lt;50%)</span>
            </div>
            <div class="flex items-center gap-3">
              <div class="h-1 w-8" style="background-color: #f59e0b"></div>
              <span class="text-sm text-gray-300">中 (50-80%)</span>
            </div>
            <div class="flex items-center gap-3">
              <div class="h-1.5 w-8" style="background-color: #ef4444"></div>
              <span class="text-sm text-gray-300">高 (&gt;80%)</span>
            </div>
            <div class="flex items-center gap-3">
        ...
        <!-- Custom Links -->
        <div>
          <p class="mb-2 text-xs font-medium text-gray-500">自定义连线</p>
          <div class="space-y-2">
            <select
              v-model="linkForm.sourceId"
              class="w-full rounded-lg border border-gray-700 bg-gray-800 px-2 py-1.5 text-xs text-gray-300 focus:outline-none focus:border-cyan-500"
            >
          ...
          <div v-if="customLinks.length" class="mt-3 space-y-1.5 max-h-44 overflow-y-auto custom-scrollbar">
            <div
              ...
              :key="l.id"
              class="flex items-center justify-between gap-2 rounded-lg bg-gray-800/60 px-2 py-1.5 text-xs"
            >
              <span class="text-gray-300 truncate">{{ l.source_name }} ↔ {{ l.target_name }}</span>
              <button
          ...
          <div v-else class="mt-3 text-xs text-gray-600">暂无自定义连线</div>
        </div>
      </div>
    </div>
  </div>
