  ...
  <div class="min-h-screen bg-gray-950 text-gray-100 p-6">
    ...
    <div class="flex flex-wrap items-end gap-4 mb-6">
      <div class="flex-1 min-w-[200px]">
        <label class="block text-sm text-gray-400 mb-1">关键词搜索</label>
        <input
          v-model="searchKeyword"
          type="text"
          placeholder="搜索设备名称、IP..."
          class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-colors"
          @keyup.enter="handleSearch"
        />
      </div>

      <div class="w-40">
        <label class="block text-sm text-gray-400 mb-1">厂商</label>
        <select
          v-model="filterVendor"
          class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500 transition-colors"
          @change="handleSearch"
        >
      ...
      <button
        class="px-4 py-2 bg-red-600/80 hover:bg-red-700 rounded-lg text-white font-medium transition-colors whitespace-nowrap"
      ...
      <button
        class="px-4 py-2 bg-gray-700/80 hover:bg-gray-600 rounded-lg text-gray-200 font-medium transition-colors whitespace-nowrap"
        :disabled="exporting"
        @click="handleExport"
      >
        {{ exporting ? '导出中...' : '⬇ 导出清单' }}
      ...
      <button
        class="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-white font-medium transition-colors whitespace-nowrap"
        ...
        <table class="w-full">
          <thead>
            <tr class="bg-gray-800/50 text-left text-sm text-gray-400">
              ...
              <th
                class="px-4 py-3 font-medium cursor-pointer select-none hover:text-gray-200 transition-colors"
          ...
          <tbody class="divide-y divide-gray-800">
            ...
            >
              ...
              <td class="px-4 py-3 text-gray-300 font-mono text-sm">{{ device.ip }}</td>
              <td class="px-4 py-3 text-gray-300">{{ device.vendor }}</td>
              <td class="px-4 py-3 text-gray-300">{{ device.model || '-' }}</td>
              <td class="px-4 py-3 text-gray-300">{{ deviceTypeLabel(device.device_type) }}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm text-gray-300">{{ device.cpu_usage ?? '-' }}%</span>
                </div>
              </td>
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm text-gray-300">{{ device.memory_usage ?? '-' }}%</span>
                </div>
              </td>
              <td class="px-4 py-3 text-sm text-gray-400">{{ formatTime(device.last_seen) }}</td>
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <router-link
                    :to="`/devices/${device.id}`"
                    class="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded text-gray-300 transition-colors"
                  >
                    编辑
                  </router-link>
                  <button
    ...
    <!-- Pagination -->
    <div v-if="total > 0" class="flex items-center justify-between mt-4 text-sm text-gray-400">
      ...
      <div class="flex items-center gap-2">
        ...
        <select
          v-model="pageSize"
          class="px-2 py-1 bg-gray-800 border border-gray-700 rounded text-gray-300 focus:outline-none focus:border-blue-500"
    ...
    <div
      ...
      class="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
    >
      <div class="bg-gray-900 border border-gray-700 rounded-xl p-6 w-[480px] shadow-2xl">
        <h3 class="text-lg font-semibold text-gray-100 mb-3">批量删除设备</h3>
        <p class="text-gray-400 mb-4">
          将删除 <span class="text-red-400 font-semibold">{{ batchDeleteTargets.length }}</span> 台设备：
          <span class="text-gray-300">{{ batchDeleteNames }}</span>
        </p>
    ...
    >
      <div class="bg-gray-900 border border-gray-700 rounded-xl p-6 w-[640px] max-h-[85vh] overflow-auto shadow-2xl">
        <h3 class="text-lg font-semibold text-gray-100 mb-4">添加设备</h3>

        <!-- Mode tabs -->
        <div class="flex gap-2 mb-4">
          <button
            class="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
            :class="addMode === 'single' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'"
          ...
          <button
            class="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
            :class="addMode === 'batch' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'"
            @click="addMode = 'batch'"
          >
            批量发现
          </button>
        ...
        <div v-if="addMode === 'single'" class="space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input v-model="singleForm.name" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="如: SW4">
            </div>
            <div>
              <label class="block text-sm text-gray-400 mb-1">IP 地址 *</label>
              <input v-model="singleForm.ip" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="如: 192.168.11.20">
            </div>
            <div>
              <label class="block text-sm text-gray-400 mb-1">厂商</label>
              <select v-model="singleForm.vendor" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500">
            ...
            <div>
              <label class="block text-sm text-gray-400 mb-1">设备类型</label>
              <select v-model="singleForm.device_type" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500">
                <option value="">未知</option>
                <option value="router">路由器</option>
                <option value="switch">交换机</option>
                <option value="firewall">防火墙</option>
            ...
            <div>
              <label class="block text-sm text-gray-400 mb-1">SNMP 团体字</label>
              <input v-model="singleForm.snmp_community" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="aiops">
            </div>
            <div>
              <label class="block text-sm text-gray-400 mb-1">SNMP 版本</label>
              <select v-model="singleForm.snmp_version" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500">
            ...
            <div>
              ...
              <input v-model="singleForm.model" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="如: S5700-28C-HI">
            </div>
            <div>
              <label class="block text-sm text-gray-400 mb-1">位置</label>
              <input v-model="singleForm.location" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="如: 机房A-机柜3">
            </div>
          </div>

          <!-- 远程管理配置 -->
          <div class="border-t border-gray-700 pt-3 mt-3">
            <div class="text-sm text-gray-400 mb-2 font-medium">远程管理配置</div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-sm text-gray-400 mb-1">管理协议</label>
                <select v-model="singleForm.mgmt_protocol" @change="onProtocolChange(singleForm)" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500">
              ...
              <div>
                ...
                <input v-model.number="singleForm.mgmt_port" type="number" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="22">
              </div>
              <div>
                <label class="block text-sm text-gray-400 mb-1">用户名</label>
                <input v-model="singleForm.mgmt_username" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="如: admin">
              </div>
              <div>
                <label class="block text-sm text-gray-400 mb-1">密码</label>
                <input v-model="singleForm.mgmt_password" type="password" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="输入管理密码">
              </div>
            </div>
          </div>
          <div class="flex justify-end gap-3 pt-2">
            <button class="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-gray-300 transition-colors" @click="showAddDialog = false">取消</button>
            <button class="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-white font-medium transition-colors" :disabled="!singleForm.name || !singleForm.ip" @click="addSingleDevice">添加设备</button>
          </div>
        </div>
        ...
        <div v-if="addMode === 'batch'" class="space-y-3">
          <div>
            <label class="block text-sm text-gray-400 mb-1">IP 列表（每行一个IP或用逗号分隔）</label>
            <textarea
              v-model="batchIpList"
              rows="5"
              class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500 font-mono text-sm"
          ...
          <div class="flex gap-3 items-end">
            <div class="flex-1">
              <label class="block text-sm text-gray-400 mb-1">SNMP 团体字</label>
              <input v-model="batchCommunity" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="aiops">
            </div>
            <button
          ...
          <div class="border-t border-gray-700 pt-3">
            <div class="text-sm text-gray-400 mb-2 font-medium">远程管理凭据（应用于所有发现的设备）</div>
            <div class="grid grid-cols-4 gap-3">
              <div>
                <label class="block text-sm text-gray-400 mb-1">管理协议</label>
                <select v-model="batchMgmt.protocol" @change="onProtocolChange(batchMgmt)" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500">
                  <option value="ssh">SSH</option>
                  <option value="telnet">Telnet</option>
                </select>
              </div>
              <div>
                <label class="block text-sm text-gray-400 mb-1">端口</label>
                <input v-model.number="batchMgmt.port" type="number" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="22">
              </div>
              <div>
                <label class="block text-sm text-gray-400 mb-1">用户名</label>
                <input v-model="batchMgmt.username" type="text" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="如: admin">
              </div>
              <div>
                <label class="block text-sm text-gray-400 mb-1">密码</label>
                <input v-model="batchMgmt.password" type="password" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 focus:outline-none focus:border-blue-500" placeholder="输入管理密码">
              </div>
            </div>
          </div>
          ...
          <div v-if="discoveredDevices.length > 0" class="mt-4">
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="flex gap-2">
                <button class="px-3 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded text-gray-300" @click="selectAllDiscovered">全选</button>
                <button
                  class="px-3 py-1 text-xs bg-blue-600 hover:bg-blue-700 rounded text-white font-medium"
            ...
            <div class="flex gap-2 mb-2">
              <select v-model="discoveryVendorFilter" class="px-3 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 text-sm focus:outline-none focus:border-blue-500">
            ...
            <div class="bg-gray-800/50 rounded-lg border border-gray-700 max-h-64 overflow-auto">
              <table class="w-full">
                <thead>
                ...
                <tbody>
                  <tr
                    ...
                    :key="d.ip"
                    class="border-b border-gray-800 text-sm hover:bg-gray-800/30"
                  >
                    <td class="px-3 py-2">
                      <input type="checkbox" :value="d.ip" v-model="selectedDiscovered" class="rounded bg-gray-700 border-gray-600">
                    </td>
                    <td class="px-3 py-2 font-mono text-gray-300">{{ d.ip }}</td>
                    <td class="px-3 py-2 text-gray-300">{{ d.name || '-' }}</td>
                    <td class="px-3 py-2 text-gray-300">{{ d.vendor || '-' }}</td>
                    <td class="px-3 py-2 text-gray-400">{{ d.model || '-' }}</td>
                    <td class="px-3 py-2 text-gray-400">{{ deviceTypeMap[d.device_type] || d.device_type || '-' }}</td>
                  </tr>
                  <tr v-if="filteredDiscovered.length === 0">
                    <td colspan="6" class="px-3 py-6 text-center text-gray-500 text-sm">无可添加的设备</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
          <div v-if="discoveryDone && discoveredDevices.length === 0" class="text-center text-gray-500 py-6">
...
function cpuBarColor(usage) {
  if (usage == null) return 'bg-gray-500'
