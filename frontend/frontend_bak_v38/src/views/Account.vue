  ...
  <div class="min-h-screen bg-gray-950 text-gray-100">
    ...
    <div class="border-b border-gray-800 bg-gray-900/50 px-6 py-4">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-100">账号管理</h1>
          <p class="mt-1 text-sm text-gray-400">修改密码、管理平台账号与权限</p>
        </div>
        <button
          @click="handleLogout"
          :disabled="loggingOut"
    ...
    <div class="px-6 py-6">
      <!-- 当前账号 -->
      <div class="mb-6 rounded-xl border border-gray-800 bg-gray-900/50 p-5">
        <h2 class="mb-4 text-sm font-semibold text-gray-300">当前账号</h2>
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="text-base font-semibold text-gray-100">{{ me.username }}</div>
            <span
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-xl border border-gray-800 bg-gray-900/50 p-5">
          <h2 class="mb-4 text-sm font-semibold text-gray-300">修改我的密码</h2>
          <form @submit.prevent="submitChangePassword" class="space-y-4">
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-400">旧密码</label>
              <input
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-400">新密码</label>
              <input
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-400">确认新密码</label>
              <input
            ...
            <button
              ...
              class="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-cyan-700 disabled:opacity-50"
            >
        ...
        <!-- 新建账号（仅 admin） -->
        <div v-if="isAdmin" class="rounded-xl border border-gray-800 bg-gray-900/50 p-5">
          <h2 class="mb-4 text-sm font-semibold text-gray-300">新建账号</h2>
          <form @submit.prevent="submitCreateUser" class="space-y-4">
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-400">用户名</label>
              <input
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-400">初始密码</label>
              <input
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-400">角色</label>
              <select
      ...
      <!-- 账号列表（仅 admin） -->
      <div v-if="isAdmin" class="mt-6 overflow-hidden rounded-xl border border-gray-800 bg-gray-900/50">
        <div class="border-b border-gray-800 px-5 py-4">
          <h2 class="text-sm font-semibold text-gray-300">账号列表</h2>
        </div>
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-gray-800">
            <tr v-for="user in users" :key="user.id" class="transition-colors hover:bg-gray-800/30">
              <td class="px-4 py-3 text-gray-400">{{ user.id }}</td>
              <td class="px-4 py-3 font-medium text-gray-100">{{ user.username }}</td>
              <td class="px-4 py-3">
                <span
              ...
              <td class="px-4 py-3">
                <span
                  :class="user.is_active ? 'bg-green-600/15 text-green-400 border-green-600/30' : 'bg-gray-600/15 text-gray-400 border-gray-600/30'"
                  class="inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium"
                >
                  {{ user.is_active ? '启用' : '停用' }}
                </span>
              </td>
              <td class="px-4 py-3 text-gray-400">{{ formatTime(user.created_at) }}</td>
              <td class="px-4 py-3">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <button
                    ...
                    :class="user.is_active ? 'text-gray-400 hover:bg-gray-700/30 hover:text-gray-200' : 'text-green-400 hover:bg-green-600/20 hover:text-green-300'"
            ...
            <tr v-if="users.length === 0">
              <td colspan="6" class="px-4 py-12 text-center text-gray-500">暂无账号</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    >
      <div class="w-full max-w-sm rounded-xl border border-gray-800 bg-gray-900 shadow-2xl">
        <div class="border-b border-gray-800 px-6 py-4">
          <h2 class="text-lg font-semibold text-gray-100">重置密码</h2>
        </div>
        <div class="px-6 py-4">
          <p class="mb-3 text-sm text-gray-400">
            为账号 <span class="font-medium text-gray-200">{{ resetTarget?.username }}</span> 设置新密码
          </p>
          <input
            v-model="resetPwd"
            type="password"
