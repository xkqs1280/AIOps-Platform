<template>
  <div class="p-6 max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-100">授权管理</h2>
        <p class="text-sm text-gray-500 mt-1">平台版本授权与激活（离线激活码，绑定本机）</p>
      </div>
    </div>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
      <div class="flex items-center gap-3 mb-5">
        ...
        <div>
          <div class="flex items-center gap-2">
            <h3 class="font-semibold text-gray-100">当前授权状态</h3>
            <span
      ...
      <div class="grid grid-cols-2 gap-4 text-sm">
        <div class="bg-gray-800/60 rounded-lg p-3">
          <div class="text-gray-500 text-xs mb-1">授权版本</div>
          <div class="text-gray-100 font-medium">{{ versionText }}</div>
        </div>
        <div class="bg-gray-800/60 rounded-lg p-3">
          <div class="text-gray-500 text-xs mb-1">到期时间</div>
          <div class="text-gray-100 font-medium">{{ expireText }}</div>
        </div>
        <div class="bg-gray-800/60 rounded-lg p-3">
          <div class="text-gray-500 text-xs mb-1">剩余天数</div>
          <div class="text-gray-100 font-medium" :class="daysLeft !== null && daysLeft <= 30 ? 'text-orange-400' : ''">
            {{ daysLeft === null ? '—' : daysLeft + ' 天' }}
          </div>
        </div>
        <div class="bg-gray-800/60 rounded-lg p-3">
          <div class="text-gray-500 text-xs mb-1">授权说明</div>
          <div class="text-gray-100 font-medium">{{ reason }}</div>
        </div>
      </div>

      <!-- 机器指纹 -->
      <div class="mt-5">
        <div class="text-gray-500 text-xs mb-1.5">本机机器码（发给厂商生成激活码）</div>
        <div class="flex items-center gap-2">
          <code class="flex-1 bg-black/40 border border-gray-700 rounded-lg px-3 py-2 text-sm text-cyan-300 select-all">
            {{ fingerprint }}
          </code>
          <button
            @click="copyFingerprint"
            class="px-3 py-2 text-xs rounded-lg bg-gray-800 border border-gray-700 text-gray-300 hover:bg-gray-700 transition-colors"
          >复制</button>
        </div>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
      <h3 class="font-semibold text-gray-100 mb-1.5">激活 / 续期授权</h3>
      ...
      <textarea
        v-model="licenseCode"
        rows="4"
        class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none font-mono"
        placeholder="粘贴激活码…"
      ></textarea>
...
const statusBadge = computed(() => {
  const s = status.value
  if (!s.enabled) return 'bg-gray-800 text-gray-400'
