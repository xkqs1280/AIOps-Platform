<template>
  <div class="min-h-screen bg-gray-950 text-gray-100 p-4 flex flex-col gap-4">
    ...
    <div v-if="activeTab === 'reminders'" class="flex flex-col gap-4">
      <div v-for="section in reminderSections" :key="section.key" class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm" v-if="getReminderList(section.key).length">
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-2.5 text-gray-200">{{ item.device_name }}</td>
                <td class="px-4 py-2.5">
                  <span class="px-2 py-0.5 rounded text-xs font-medium" :class="section.badgeBg">{{ item.days_remaining }} 天</span>
                </td>
                <td class="px-4 py-2.5 text-gray-400">{{ item.date }}</td>
              </tr>
            </tbody>
          </table>
          <div v-else class="px-4 py-10 text-center text-gray-500 text-sm">暂无数据</div>
        </div>
      </div>
    </div>

    <!-- Tab 2: 厂商生命周期数据库 -->
    <div v-if="activeTab === 'eos'" class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm" v-if="eosDb.length">
          ...
          <tbody>
            <tr
              ...
              class="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors"
            >
              <td class="px-4 py-2.5 text-gray-200">{{ item.vendor }}</td>
              <td class="px-4 py-2.5 text-gray-300 font-mono text-xs">{{ item.model }}</td>
              <td class="px-4 py-2.5 text-gray-400">{{ item.eos_date }}</td>
              <td class="px-4 py-2.5 text-gray-400">{{ item.eol_date }}</td>
              <td class="px-4 py-2.5 text-gray-500 text-xs">{{ item.source }}</td>
        ...
        <div v-else class="px-4 py-10 text-center text-gray-500 text-sm">暂无数据，请添加或点击"种子数据"</div>
      </div>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="closeModal">
      <div class="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-lg p-6 shadow-2xl max-h-[85vh] overflow-auto custom-scrollbar">
        <h3 class="text-lg font-semibold mb-4">{{ editingItem ? '编辑记录' : '添加记录（选择设备）' }}</h3>
        <div class="space-y-3">
          <!-- 编辑模式：厂商/型号直接输入 -->
          <template v-if="editingItem">
            <div>
              <label class="text-xs text-gray-400 mb-1 block">厂商</label>
              <input v-model="form.vendor" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none" placeholder="如: H3C" />
            </div>
            <div>
              <label class="text-xs text-gray-400 mb-1 block">型号</label>
              <input v-model="form.model" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none" placeholder="如: S5130S-28P-EI" />
            </div>
          </template>
          ...
          <template v-else>
            <div>
              <label class="text-xs text-gray-400 mb-1 block">选择已纳管设备（可多选）</label>
              <input
                v-model="deviceFilter"
                class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none mb-1.5"
                placeholder="搜索设备名 / IP / 厂商 / 型号..."
              />
              <div class="max-h-40 overflow-auto border border-gray-700 rounded-lg bg-gray-800/40 divide-y divide-gray-800 custom-scrollbar">
                <label
                  v-for="d in filteredDevices"
                  :key="d.id"
                  class="flex items-center gap-2.5 px-3 py-1.5 cursor-pointer hover:bg-gray-700/40"
                >
                  ...
                  <span class="text-xs text-gray-500 font-mono">{{ d.ip }}</span>
                  <span v-if="d.vendor || d.model" class="text-xs text-gray-600 ml-auto">
                    {{ d.vendor || '?' }} {{ d.model || '未识别型号' }}
                  </span>
                </label>
                <div v-if="filteredDevices.length === 0" class="px-3 py-4 text-center text-xs text-gray-500">无匹配设备</div>
              </div>
              <div class="mt-1 text-xs text-gray-500">已选 {{ selectedDeviceIds.length }} 台</div>
            </div>
            ...
            <div v-if="devicePreview.length" class="bg-blue-950/20 border border-blue-900/40 rounded-lg p-3">
              ...
              <div v-for="p in devicePreview" :key="`${p.vendor}|${p.model}`" class="text-xs text-gray-300 py-0.5">
                {{ p.vendor || '未识别厂商' }} <span class="text-gray-500">/</span> {{ p.model || '未识别型号' }}
                <span class="text-gray-500">× {{ p.count }} 台</span>
              </div>
          ...
          <div>
            <label class="text-xs text-gray-400 mb-1 block">维保到期日期</label>
            <input v-model="form.eos_date" type="date" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none" />
          </div>
          <div>
            <label class="text-xs text-gray-400 mb-1 block">设备寿命到期日期（一般按 8 年）</label>
            <input v-model="form.eol_date" type="date" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none" />
          </div>
          <div>
            <label class="text-xs text-gray-400 mb-1 block">来源</label>
            <input v-model="form.source" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:border-cyan-500 focus:outline-none" placeholder="如: H3C官方" />
          </div>
        </div>
        <div class="flex justify-end gap-2 mt-5">
          <button @click="closeModal" class="px-4 py-2 text-sm rounded-lg bg-gray-800 text-gray-400 hover:text-gray-200 transition-colors">取消</button>
...
const reminderSections = [
  ...
  },
  {
    ...
    badgeBg: 'bg-gray-600/30 text-gray-400',
  },
]
