    ...
    <div class="w-full max-w-sm">
      ...
      >
        <div class="flex flex-col items-center mb-6">
          ...
          <h1 class="text-xl font-bold text-cyan-400 mt-3">AIOps 平台登录</h1>
          <p class="text-xs text-gray-500 mt-1">网络及安全设备智能运维托管平台</p>
        </div>

        <label class="block text-xs text-gray-500 font-medium mb-1.5">用户名</label>
        <input
          v-model="username"
          type="text"
          placeholder="用户名"
          autocomplete="username"
          class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-sm text-gray-100
                 placeholder-gray-600 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500"
        />

        <label class="block text-xs text-gray-500 font-medium mt-4 mb-1.5">密码</label>
        <input
          v-model="password"
          type="password"
          placeholder="密码"
        ...
        <button
          type="submit"
          :disabled="loading"
          class="w-full mt-6 py-2.5 rounded-lg bg-cyan-700 hover:bg-cyan-600 disabled:opacity-60
