<template>
  <div class="p-4 pb-8">
    ...
    <h1 class="mb-3 text-lg font-bold text-ink-strong">等保合规</h1>

    <div v-if="summary" class="mb-4 rounded-2xl border border-line bg-surface p-4">
      <div class="mb-2 flex items-center justify-between">
        <span class="text-xs text-ink-faint">整体合规评分</span>
      ...
      <div class="mt-3 flex gap-3 text-center">
        <div class="flex-1 rounded-xl bg-surface-2/60 py-2">
          <p class="text-sm font-bold text-ink-strong">{{ summary.passed ?? summary.pass_count ?? 0 }}</p>
          <p class="text-[10px] text-ink-faint">通过</p>
        </div>
        <div class="flex-1 rounded-xl bg-surface-2/60 py-2">
          <p class="text-sm font-bold text-amber-400">{{ summary.warnings ?? summary.warning_count ?? 0 }}</p>
          <p class="text-[10px] text-ink-faint">警告</p>
        </div>
        <div class="flex-1 rounded-xl bg-surface-2/60 py-2">
          <p class="text-sm font-bold text-red-400">{{ summary.failed ?? summary.fail_count ?? 0 }}</p>
          <p class="text-[10px] text-ink-faint">不通过</p>
        </div>
      </div>
    </div>

    <div v-if="items.length" class="space-y-2">
      <div v-for="it in items" :key="it.id ?? it.check_item" class="flex items-center gap-3 rounded-2xl border border-line bg-surface p-3">
        <span class="text-lg">{{ statusIcon(it.status ?? it.result) }}</span>
        <div class="min-w-0 flex-1">
          <p class="truncate text-sm text-ink">{{ it.check_item || it.item || it.name }}</p>
          <p class="text-xs text-ink-faint">{{ it.device_name || '' }}</p>
        </div>
      </div>
    </div>
    <div v-else-if="!loading" class="py-16 text-center text-sm text-ink-faint">暂无合规数据</div>
