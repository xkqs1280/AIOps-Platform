<template>
  <div class="p-4">
    <header class="mb-3 flex items-center justify-between">
      <h1 class="text-lg font-bold text-slate-100">设备管理</h1>
      <span class="text-xs text-slate-500">共 {{ total }} 台</span>
    </header>
    ...
    <div class="mb-3 flex gap-2">
      <input
        ...
        class="flex-1 rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-sm text-slate-100 outline-none placeholder:text-slate-600 focus:border-cyan-500"
      />
    ...
    <div v-if="devices.length" class="space-y-2.5">
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <p class="truncate text-sm font-medium text-slate-100">{{ d.name }}</p>
          <p class="truncate text-xs text-slate-500">{{ d.ip }} · {{ d.model || d.vendor || '—' }}</p>
        </div>
    ...
    <div v-else class="py-16 text-center text-sm text-slate-600">{{ loading ? '加载中...' : '暂无设备' }}</div>
  </div>
</template>
...
const statusDot = (s) => ({ online: 'bg-emerald-400', warning: 'bg-amber-400', offline: 'bg-red-400' }[s] || 'bg-slate-500')
const statusText = (s) => ({ online: '在线', warning: '告警', offline: '离线' }[s] || s || '—')
const statusTextColor = (s) => ({ online: 'text-emerald-400', warning: 'text-amber-400', offline: 'text-red-400' }[s] || 'text-slate-400')
