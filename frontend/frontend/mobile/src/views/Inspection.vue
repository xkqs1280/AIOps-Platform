<template>
  <div class="p-4 pb-8">
    ...
    <h1 class="mb-3 text-lg font-bold text-ink-strong">设备巡检</h1>

    <div v-if="tasks.length" class="space-y-2.5">
      <div v-for="t in tasks" :key="t.id" class="rounded-2xl border border-line bg-surface p-3.5">
        <div class="flex items-center justify-between">
          <p class="text-sm font-medium text-ink-strong">{{ t.task_name || t.name || '巡检任务' }}</p>
          <span class="rounded px-1.5 py-0.5 text-[10px]" :class="statusBadge(t.status)">{{ t.status || '—' }}</span>
        </div>
        <p class="mt-1 text-xs text-ink-faint">{{ t.device_name || '' }}</p>
        ...
        <p class="mt-1.5 text-xs text-ink-faint">{{ fmtTime(t.created_at || t.start_time) }}</p>
      </div>
    </div>
    <div v-else class="py-16 text-center text-sm text-ink-faint">{{ loading ? '加载中...' : '暂无巡检任务' }}</div>
  </div>
</template>
...
const statusBadge = (s) => ({ completed: 'bg-emerald-500/15 text-emerald-400', running: 'bg-cyan-500/15 text-cyan-400', failed: 'bg-red-500/15 text-red-400', pending: 'bg-amber-500/15 text-amber-400' }[s] || 'bg-ink-faint/15 text-ink-muted')
