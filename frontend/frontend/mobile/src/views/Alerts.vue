<template>
  <div class="p-4">
    <header class="mb-3 flex items-center justify-between">
      <h1 class="text-lg font-bold text-ink-strong">告警管理</h1>
      <div class="flex gap-2">
        <button @click="load(true)" class="rounded-lg border border-line px-3 py-1.5 text-xs text-ink-muted">刷新</button>
    ...
    <div v-if="alerts.length" class="space-y-2.5">
      <div v-for="a in alerts" :key="a.id" class="rounded-2xl border border-line bg-surface p-3.5">
        <div class="flex items-start gap-2.5">
          <span class="mt-1 h-2 w-2 shrink-0 rounded-full" :class="sevDot(a.severity)"></span>
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-ink-strong">{{ a.message || a.alert_name }}</p>
...
const sevBadge = (s) => ({ critical: 'bg-red-500/15 text-red-400', major: 'bg-amber-500/15 text-amber-400', minor: 'bg-yellow-500/15 text-yellow-400', warning: 'bg-cyan-500/15 text-cyan-400' }[s] || 'bg-ink-faint/15 text-ink-muted')
