<template>
  <div class="p-4">
    <header class="mb-4 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-bold text-slate-100">监控概览</h1>
        <p class="text-xs text-slate-500">{{ serverText }}</p>
      </div>
      <button @click="load" class="rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-300 active:bg-slate-800">
        刷新
      </button>
    </header>
    ...
    <div class="mb-4 rounded-2xl border border-slate-800 bg-slate-900 p-4">
      <div class="mb-2 flex items-center justify-between">
        <span class="text-xs text-slate-500">设备在线率</span>
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900">
      <div class="flex items-center justify-between border-b border-slate-800 px-4 py-3">
        <span class="text-sm font-semibold text-slate-200">最近告警</span>
        <router-link to="/alerts" class="text-xs text-cyan-400">全部 ›</router-link>
      </div>
      <div v-if="recentAlerts.length === 0" class="px-4 py-6 text-center text-xs text-slate-600">暂无告警</div>
      <div v-for="a in recentAlerts" :key="a.id" class="flex items-center gap-3 border-b border-slate-800/60 px-4 py-3 last:border-0">
        <span class="h-2 w-2 shrink-0 rounded-full" :class="severityDot(a.severity)"></span>
        <div class="min-w-0 flex-1">
          <p class="truncate text-sm text-slate-200">{{ a.message || a.alert_name }}</p>
          <p class="text-xs text-slate-500">{{ a.device_name || '' }} · {{ fmtTime(a.triggered_at || a.created_at) }}</p>
        </div>
