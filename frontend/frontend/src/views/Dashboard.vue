<template>
  <div class="min-h-screen bg-app text-ink-strong p-4 flex flex-col gap-4 animate-in">
    ...
    <div class="grid grid-cols-12 gap-4 h-48">
      ...
      <div class="col-span-2 card card-lift p-4 flex flex-col">
        ...
        <div class="grid grid-cols-2 gap-2 flex-1 mt-3">
          ...
          <div class="bg-red-900/20 rounded-lg p-2 flex flex-col items-center justify-center border border-red-800/30 transition-all duration-150 hover:bg-red-800/25 hover:-translate-y-0.5">
      ...
      <div class="col-span-3 card card-lift p-4 flex flex-col">
      ...
      <div class="col-span-4 card card-lift p-4 flex flex-col">
    ...
    <div class="grid grid-cols-12 gap-4 h-80">
      ...
      <div class="col-span-9 card card-lift p-4 flex flex-col">
        <PanelTitle title="网络拓扑图" accent="cyan" />
        <div ref="topologyChartRef" class="flex-1 w-full"></div>
      </div>

      <!-- Memory Usage TOP5 -->
      <div class="col-span-1 card card-lift p-4 flex flex-col">
...
const PanelTitle = {
  props: ['title'],
  setup(props) {
    // 统一品牌色指示条（收敛原先每个卡片不同的 accent 色，保持克制）
    return () => h('div', {
